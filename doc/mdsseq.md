...
- `f3 nn ii [ii] ...` - `rndpat`
	Random pattern selection. Randomly calls one of the specified
	subroutines each time the command is executed. `nn` is the number
	of choices (2-8), followed by `nn` subroutine index bytes. Each `ii`
	is an index into the song data table (like `pat`). This enables
	variable/dynamic music patterns.
