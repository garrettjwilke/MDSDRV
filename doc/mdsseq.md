...
- `f2 dd(ub)` - `pcmmode`
	Set the PCM driver mixing mode. Valid values are `2..4`.
	A value of `2` sets the 2 channel mixing mode at ~17 kHz.
	A value of `3` sets the 3 channel mixing mode at 13 kHz.
	A value of `4` is mode 2 with SSDPCM (ss2) on PCM1 (track F)
	and raw PCM on PCM2 (track K). `'pcmrate'` and volume envelopes
	work the same as mode 2.
...
- `+0 (ub)` - Sample rate
	This value must be between 1 and 8. Defines sample rate in
	~2190 Hz steps (mode 2/4 mix clock / 8).
...
When byte `+0` has bit 7 set, the header describes an SSDPCM stream
used with `pcmmode 4` on PCM1:

- `+0 (ub)` - `0x80 | pitch` where pitch is `1..8` (same meaning as
	raw PCM / `'pcmrate'`)
- `+1 (ub)` - Address bits 23-16 (into `mdspcm.bin`)
- `+2 (uw)` - Address bits 15-0
- `+4 (uw)` - Block count (little-endian; 34 bytes and 128 samples per
	block, after a 1-byte initial sample)
- `+6 (ub)` - Loop flag (non-zero = loop)
- `+7 (ub)` - Reserved
